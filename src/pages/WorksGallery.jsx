...
function WorksGallery({ currentLocation, filterOptions = { season: [], festival: [], material_type: [] } }) {
  ...
  useEffect(() => {
    if (currentLocation && works.length > 0) {
      loadWorkStatistics()
    } else {
      setWorkStats({})
    }
  ...
  const loadWorkStatistics = async () => {
    ...
    await Promise.all(works.map(async (work) => {
      try {
        const stat = await teachingRecordsAPI.getWorkStatistics(work.id, currentLocation.id)
        stats[work.id] = stat
      } catch (error) {
    ...
    <div className="pb-24">
      <div className="mb-8">
        ...
        <p className="text-gray-600 dark:text-gray-400 text-lg">
          {currentLocation ? `📍 ${currentLocation.name}` : '請先選擇活動中心以查看教學記錄'}
        </p>
      </div>
...
function FilterGroup({ label, options, selected, onChange }) {
  if (!options || options.length === 0) return (
    <div>
      <label className="text-xs font-medium text-gray-400 dark:text-gray-500 mb-1 block">{label}</label>
      <p className="text-xs text-gray-400 dark:text-gray-500 italic">（無選項）</p>
    </div>
  )
    ...
    <div className="group bg-white/80 dark:bg-gray-800/80 backdrop-blur-lg rounded-2xl shadow-lg overflow-hidden hover:shadow-2xl transition-all duration-300 hover:scale-[1.02] border border-gray-200 dark:border-gray-700">
      <Link to={`/work/${work.id}`} className="block">
        <div className="aspect-[4/3] bg-gradient-to-br from-gray-200 to-gray-300 dark:from-gray-700 dark:to-gray-600 relative overflow-hidden">
          ...
          <button
            ...
            className="absolute top-3 left-3 bg-red-500 hover:bg-red-600 text-white p-2 rounded-full shadow-lg opacity-0 group-hover:opacity-100 transition-opacity duration-200"
          >
            🗑️
          </button>
        </div>
      ...
      <div className="p-4">
        ...
        <div className="flex flex-wrap gap-1.5 mb-3">
          {work.season && (
            <span className="text-xs px-2 py-1 bg-green-100 dark:bg-green-900/30 text-green-700 dark:text-green-400 rounded-md font-medium">{work.season}</span>
          )}
        ...
        {currentLocation && stats && (
          <div className="text-xs text-gray-500 dark:text-gray-400 space-y-1 mb-3 bg-gray-50 dark:bg-gray-700/50 rounded-lg p-2">
            <p className="flex items-center gap-1"><span>📊</span><span>在此中心教過 {stats.total_times} 次</span></p>
            {stats.last_taught && (
              <p className="flex items-center gap-1"><span>📅</span><span>上次：{format(new Date(stats.last_taught), 'yyyy/MM/dd')}</span></p>
            )}
          </div>
        )}
        <Link
      ...
      {showDeleteConfirm && (
        <div className="fixed inset-0 bg-black/50 backdrop-blur-sm flex items-center justify-center z-50 p-4">
          <div className="bg-white dark:bg-gray-800 rounded-2xl p-6 max-w-sm w-full shadow-2xl">
            <h3 className="text-lg font-bold text-gray-900 dark:text-white mb-2">確定要刪除嗎？</h3>
            <p className="text-gray-600 dark:text-gray-400 mb-6">刪除「{work.title}」後將無法復原，相關的教學記錄也會被刪除。</p>
            <div className="flex gap-3">
              ...
              <button onClick={() => { setShowDeleteConfirm(false); onDelete(work.id) }}
